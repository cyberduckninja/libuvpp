...
namespace libuvpp {
    ...
    protected:
        stream() : handle<HANDLE_T>() { }

    public:
        bool listen(CallbackWithResult callback, int backlog = 128) {
            callbacks::store(
                    handle<HANDLE_T>::get()->data,
                    libuvpp::internal::uv_cid_listen,
                    callback
            );
            return uv_listen(
                    handle<HANDLE_T>::template get<uv_stream_t>(),
                    backlog,
        ...
        bool write(const char *buf, int len, CallbackWithResult callback) {
            ...
            callbacks::store(
                    handle<HANDLE_T>::get()->data,
                    libuvpp::internal::uv_cid_write,
                    callback
            );
            return uv_write(
                    new uv_write_t, handle<HANDLE_T>::template get<uv_stream_t>(),
                    bufs,
                    1,
        ...
        bool shutdown(CallbackWithResult callback) {
            callbacks::store(
                    handle<HANDLE_T>::get()->data,
                    libuvpp::internal::uv_cid_shutdown,
                    callback
            );
            return uv_shutdown(
                    new uv_shutdown_t, handle<HANDLE_T>::template get<uv_stream_t>(),
